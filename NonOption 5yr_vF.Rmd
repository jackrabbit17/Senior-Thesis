...
fan = rep(NA,dim(total)[1]-252*5)
for(i in 1:(dim(total)[1]-252*5)){
  fan[i] = (total[i+252*5,]$SPY.Adjusted/total[i,]$SPY.Adjusted)^(1/5)-1
}
hist(fan)
...
fan6040 = matrix(data=NA,nrow=252*5,ncol=dim(total)[1]-252*5)
for(j in 1:(dim(total)[1]-252*5)){
  #Initialize values
  spy.portval = 60
  bond.portval = 40
...
#Calculate returns
fan6040.returns = rep(NA,(dim(total)[1]-252*5))
for(j in 1:(dim(total)[1]-252*5)){
  fan6040.returns[j] = (fan6040[252*5,j]/fan6040[1,j])^(1/5)-1
}
...
fan.diversified = matrix(data=NA,nrow=252*5,ncol=dim(total)[1]-252*5)
...
#Calculate returns
fan.diversified.returns = rep(NA,(dim(total)[1]-252*5))
for(j in 1:(dim(total)[1]-252*5)){
  fan.diversified.returns[j] = (fan.diversified[252*5,j]/fan.diversified[1,j])^(1/5)-1
}
...
fan.total.diversified = matrix(data=NA,nrow=252*5,ncol=dim(total)[1]-252*5)
...
#Calculate returns
fan.total.diversified.returns = rep(NA,(dim(total)[1]-252*5))
for(j in 1:(dim(total)[1]-252*5)){
  fan.total.diversified.returns[j] = (fan.total.diversified[252*5,j]/fan.total.diversified[1,j])^(1/5)-1
}
...
fan.vol = matrix(data=NA,nrow=252*5,ncol=dim(total)[1]-252*5)
...
#Calculate returns
fan.vol.returns = rep(NA,(dim(total)[1]-252*5))
for(j in 1:(dim(total)[1]-252*5)){
  fan.vol.returns[j] = (fan.vol[252*5,j]/fan.vol[1,j])^(1/5)-1
}
...
```{r}
#Stop Loss
fan.stop = matrix(data=NA,nrow=252*5,ncol=dim(total)[1]-252*5)
limit = 0.1

for(j in 22:(dim(total)[1]-252*5)){
  spy.portval = 100
  cash.portval = 0
  protection = 0
...
#Calculate returns
fan.stop.returns = rep(NA,(dim(total)[1]-252*5))
for(j in 22:(dim(total)[1]-252*5)){
  fan.stop.returns[j] = (fan.stop[252*5,j]/fan.stop[1,j])^(1/5)-1
}
...
lines(density(fiveyear.return1,na.rm=T),col="blue")

plot(density(fan),main="Tail Risk 2",xlab="10-Year Rolling Returns",ylab="Density",ylim=c(0,30))
lines(density(fiveyear.return,na.rm=T),col="blue")
...
```{r}
barplot(c(mean(fan6040.returns > fan,na.rm=T),mean(fan.diversified.returns > fan,na.rm=T),mean(fan.total.diversified.returns > fan,na.rm=T),mean(fan.vol.returns > fan,na.rm=T),mean(fiveyear.return1 > cagr.spy,na.rm=T),mean(fiveyear.return[2:19] > cagr.spy,na.rm=T),mean(fan.stop.returns > fan,na.rm=T)),names.arg=bar.names[2:length(bar.names)],main="Fraction of 5-Year Annualized Returns Exceeding Market Benchmark",col=c("grey","tomato","orange","purple","green","turquoise","violet"))
